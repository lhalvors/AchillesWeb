define(["jquery", "d3", "jnj/chart", "common", "datatables", "datatables-colvis"], function ($, d3, jnj_chart, common) {
	...
	$(document).on('click', '#sb_fact_table tbody tr', function () {
		$('#sb_fact_table tbody tr.selected').removeClass('selected');
		$(this).addClass('selected');
		id = datatable.data()[datatable.row(this)[0]].concept_id;
		concept_name = datatable.data()[datatable.row(this)[0]].fact_name;
		sb_fact_report.drilldown(id, concept_name);
	});
	...
	sb_fact_report.drilldown = function (concept_id, concept_name) {
		$('.drilldown svg').remove();
		$('#factDrilldownTitle').text(concept_name);
		$('#reportFactDrilldown').removeClass('hidden');

		$.ajax({
			type: "GET",
			url:getUrlFromDataCollection(page_vm.datasource(), "sb_facts", concept_id),
			success: function (data) {
				// age at first fact visualization
				...
				ageAtFirstOccurrence.render(bpseries, "#reportFacts #ageAtFirstOccurrence", 500, 300, {
					xLabel: 'Gender',
				...
				// prevalenceByMonth.render(byMonthSeries, "#reportFacts #factPrevalenceByMonth", 1000, 300, {
				...
				// fact type visualization
				var factsByType = new jnj_chart.donut();
				factsByType.render(common.mapConceptData(data.FACTS_BY_TYPE), "#reportFacts #factsByType", 500, 300, {
					margin: {
						top: 5,
						left: 5,
						right: 220,
				...
				// chart.render(dataByDecile, "#reportFacts #trellisLinePlot", 1000, 300, {
					// trellisSet: allDeciles,
					// trellisLabel: "Age Decile",
					// seriesLabel: "Year of Fact",
					// yLabel: "Prevalence Per 1000 People",
				...
				recordsByUnit.render(datdaRecordsByUnit, "#reportFacts #recordsByUnit", 500, 300, {
					margin: {
						top: 5,
						left: 5,
						right: 200,
				...
				// Fact Value Distribution
				var factValues = new jnj_chart.boxplot();
				bpseries = [];
				bpdata = common.normalizeDataframe(data.FACT_VALUE_DISTRIBUTION);
				...
				factValues.render(bpseries, "#reportFacts #factValues", 500, 300, {
				...
				lowerLimit.render(bpseries, "#reportFacts #lowerLimit", 300, 200, {
					yMax: d3.max(bpdata.P90_VALUE) || bpdata.P90_VALUE, // handle when dataframe is not array of values
					xLabel: 'Unit',
				...
				upperLimit.render(bpseries, "#reportFacts #upperLimit", 300, 200, {
					yMax: d3.max(bpdata.P90_VALUE) || bpdata.P90_VALUE, // handle when dataframe is not array of values
					xLabel: 'Unit',
					yLabel: 'Measurement Value'
				});
				...
				relativeToNorm.render(dataRelativeToNorm, "#reportFacts #relativeToNorm", 500, 300, {
					margin: {
						top: 5,
						left: 5,
						right: 200,
	...
	sb_fact_report.render = function (datasource) {
		...
		$.ajax({
			type: "GET",
			url: getUrlFromData(datasource, "fact_treemap"),
			...
			success: function (data) {
				...
				datatable = $('#sb_fact_table').DataTable({
					order: [ 5, 'desc' ],
					dom: 'Clfrtip',
					data: table_data,
					columns: [
				...
				treemap.render(tree, '#reportFacts #treemap_container', width, height, {
					onclick: function (node) {
						sb_fact_report.drilldown(node.id, node.name)
					},
